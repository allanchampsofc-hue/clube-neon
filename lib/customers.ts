...
/**
 * Monta o filtro pro .or() do PostgREST buscando por nome, CPF, telefone ou
 * código de membro. Remove vírgula/parênteses do input pra não quebrar (ou
 * vazar) a sintaxe do filtro.
 */
export function buildCustomerSearchFilter(rawQuery: string): string {
  ...
  const parts = [`name.ilike.%${safe}%`, `member_number.ilike.%${safe}%`];
